...
int main(int argc,char **argv)
{
	...
	scanf("%c",&cData); // 后一个%*c吃掉回车符，%*c不赋值任何变量,为调用CreateBiTree()做准备
	AssignToLinkBiTreeNode(ptLinkBiTreeNode,cData);//"*"表示该输入项读入后不赋予任何变量，即跳过该输入值
	printf("层序遍历二叉树:\n");
	LevelOrderTraverseLinkBiTree(ptLinkBiTreeNode);

	cOtherData=GetLinkBiTreeParentData(ptLinkBiTreeNode,cData);
	if(cOtherData!=NIL)
	printf("%c的双亲是%c\n",cData,cOtherData);
	else
	printf("%c没有双亲\n",cData);
	cOtherData=GetLinkBiTreeLeftChildData(ptLinkBiTreeNode,cData);
	if(cOtherData!=NIL)
	printf("%c的左孩子是%c\n",cData,cOtherData);
	else
	printf("%c没有左孩子\n",cData);
	cOtherData=GetLinkBiTreeRightChildData(ptLinkBiTreeNode,cData);
	if(cOtherData!=NIL)
	printf("%c的右孩子是%c\n",cData,cOtherData);
	else
	printf("%c没有右孩子\n",cData);
	cOtherData=GetLinkBiTreeLeftSiblingData(ptLinkBiTreeNode,cData);
	if(cOtherData!=NIL)
	printf("%c的左兄弟是%c\n",cData,cOtherData);
	else
	printf("%c没有左兄弟\n",cData);
	cOtherData=GetLinkBiTreeRightSiblingData(ptLinkBiTreeNode,cData);
	if(cOtherData!=NIL)
	printf("%c的右兄弟是%c\n",cData,cOtherData);
	else
	printf("%c没有右兄弟\n",cData);
...
static int CreateLinkBiTree(T_LinkBiTreeNode **i_pptLinkBiTree)
{
	...
	{
		*i_pptLinkBiTree=(T_LinkBiTreeNode *)malloc(T_LinkBiTreeNode);
...
static void LevelOrderTraverseLinkBiTree(T_LinkBiTreeNode *i_ptLinkBiTree)
{
	T_LinkQueue *ptLinkQueue=NULL;
	T_LinkBiTreeNode *ptBiTreeNode=NULL;
	T_QueueDataElement tQueueElement;
	InitLinkQueue(ptLinkQueue);
	tQueueElement.ptData=i_ptLinkBiTree;
	EnterLinkQueue(ptLinkQueue,tQueueElement);
	while(0==ExitLinkQueue(ptLinkQueue,&tQueueElement))
	{
		...
		else
		{
			...
			EnterLinkQueue(ptLinkQueue,tQueueElement);//左右孩子入队后，如果该结点还有兄弟没处理，
		}//接下来处理的就是之前入队的兄弟结点
	}
	printf("\r\n");
}
...
static T_LinkBiTreeNode *GetLinkBiTreeNodePoint(T_LinkBiTreeNode *i_ptLinkBiTree,char i_cData)
{
	...
	else
	{
		InitLinkQueue(ptLinkQueue);
		tQueueElement.ptData=i_ptLinkBiTree;
		EnterLinkQueue(ptLinkQueue,tQueueElement);// 根指针入队
		while(0==ExitLinkQueue(ptLinkQueue,&tQueueElement))// 队不空// 出队
		{
			if(i_cData==tQueueElement.ptData->cData)
			{
				ptLinkBiTreeNode=tQueueElement.ptData;
				break;
			}
			else
			{
				if(NULL==tQueueElement.ptData->ptLeftChild)
				{
				}
				else
				{
					tQueueElement.ptData=tQueueElement.ptData->ptLeftChild;
					EnterLinkQueue(ptLinkQueue,tQueueElement);// 入队左孩子
				}
				if(NULL==tQueueElement.ptData->ptRightChild)
				{
				}
				else
				{
					tQueueElement.ptData=tQueueElement.ptData->ptRightChild;
					EnterLinkQueue(ptLinkQueue,tQueueElement);// 入队右孩子
				}
			}
		}
	}
...
static char GetLinkBiTreeParentData(T_LinkBiTreeNode *i_ptLinkBiTree,char i_cData)
{
	...
	else
	{
		...
		while(0==ExitLinkQueue(ptLinkQueue,&tQueueElement))// 队不空// 出队
		{
			if((NULL!=tQueueElement.ptData->ptLeftChild)&&(i_cData==tQueueElement.ptData->ptLeftChild->cData)
			     ||(NULL!=tQueueElement.ptData->ptRightChild)&&(i_cData==tQueueElement.ptData->ptRightChild->cData))
			{
				cData=tQueueElement.ptData;
				break;
			}
			else
			{
